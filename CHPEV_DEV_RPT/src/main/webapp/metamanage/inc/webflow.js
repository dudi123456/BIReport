...
function redrawVML(){
   ...
   if(FlowXML.value!=''){
	   ...
	   for ( var i = 0;i < Steps.childNodes.length;i++ )
	   {
		   ...
		   var title = Step.getElementsByTagName("BaseProperties").item(0).getAttribute("title");
		   ...
		   vmlHTML+= getStepHTML(nType,nIndex,nId,nText,title,nTextColor,nStrokeColor,nShadowColor,nIsShadow,nWidth,nHeight,nX,nY,nStrokeWeight,nTextWeight,nColor1,nColor2,nIs3D,n3DDepth);
	   } 
	   clearVML();
	   drawObject(vmlHTML);
	   vmlHTML = '';
	   ...
	   for ( var i = 0;Actions != null && i < Actions.childNodes.length;i++ )
	   {
		   Action = Actions.childNodes.item(i);
...
function getStepHTML(stepType,z_index,id,text,title,textColor,strokeColor,shadowColor,isShadow,width,height,X,Y,strokeWeight,textWeight,color1,color2,is3D,step3DDepth){
   var stepHTML = '';
//   var styleHTML = 'id='+id+' title="'+title+'" style="z-index:'+z_index+';position:absolute;width:'+width+';height:'+height+';left:'+X+';top:'+Y+';" strokecolor="'+strokeColor+'" strokeweight="'+strokeWeight+'" onmouseover=\'this.style.cursor="hand";\' onmousedown=\'cleancontextMenu();objFocusedOn(this.id,"'+stepType+'");moveStep(this);\' oncontextmenu=\'objFocusedOn(this.id,"'+stepType+'");stepContextMenu("'+id+'","'+stepType+'");return false;\'';
  
   var styleHTML = 'id='+id+' title="'+title+'" style="z-index:'+z_index+';position:absolute;width:'+width+';height:'+height+';left:'+X+';top:'+Y+';" strokecolor="'+strokeColor+'" strokeweight="'+strokeWeight+'" onmouseover=\'this.style.cursor="hand";\' onmousedown=\'cleancontextMenu();objFocusedOn(this.id,"'+stepType+'");moveStep(this);\' oncontextmenu=\'objFocusedOn(this.id,"'+stepType+'");stepContextTable("'+title+'");return false;\'';
...
function getActionHTML(actionType,z_index,id,actionText,fromStep,toStep,strokeColor,strokeWeight,startArrow,endArrow){
   var actionHTML = '';
//oncontextmenu=\'objFocusedOn(this.id,"Action");actionContextMenu("'+id+'");return false;\'
   if(actionType == 'Line'){
      actionHTML = '<v:line id='+id+' title="'+id+'" style="z-index:'+z_index+';position:absolute;" '+getActionPoints(actionType,fromStep,toStep)+' strokecolor="'+strokeColor+'" strokeweight="'+strokeWeight+'" onmousedown=\'cleancontextMenu();objFocusedOn(this.id,"Action");\' ;return false;\'><v:stroke StartArrow="'+startArrow+'" EndArrow="'+endArrow+'"/></v:line>';
   }else{
      actionHTML = '<v:PolyLine id='+id+' title="'+id+'" filled="false" Points="'+getActionPoints(actionType,fromStep,toStep)+'" style="z-index:'+z_index+';position:absolute;" strokecolor="'+strokeColor+'" strokeweight="'+strokeWeight+'" onmousedown=\'cleancontextMenu();objFocusedOn(this.id,"Action");\' ><v:stroke StartArrow="'+startArrow+'" EndArrow="'+endArrow+'"/></v:PolyLine>';
   }
...
var tableId;
function stepContextTable(id) {
	tableId = id;
	SourceFlow.isExistTable("'"+id+"'",showField);
}
function showField(data) {
	var tempId = tableId.replace(".","");
	if(data) {
		window.open("viewField.jsp?table_id="+tableId,"数据表字段信息"+tempId,"top=250,left=200,width=400,height=250,toolbar=0,location=0,directories=0,status=0,menubar=0,scrollbars=yes,resizable=0");
	}
}
