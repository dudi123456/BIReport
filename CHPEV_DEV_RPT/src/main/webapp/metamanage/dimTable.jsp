...
//		alert(o_Str);
  ...
  function _frmSubmit(){
	...
	function menuChanger(contentID,obj){
		...
		if(contentID==1) {
			obj.href="<%=request.getContextPath()%>/metamanage/dimDef.rptdo?dim_id=<%=dim_id%>";
		}
	}
</SCRIPT>
</HEAD>
...
<table width="99" border="0" cellspacing="0" cellpadding="0">
  <tr> 
    ...
    <tr> 
          ...
          <td nowrap class="tab-button-off"><a href="javascript:void(0)" onclick="menuChanger(1,this)" class="tab-button-off-link">维度管理</a></td>
    ...
    <td><table border="0" cellspacing="0" cellpadding="0">
        <tr> 
          <td width="5"><img src="<%=request.getContextPath()%>/images/common/taber/tab_left_off.gif" width="5" height="22"></td>
          <td nowrap class="tab-button-on"><a href="javascript:void(0)" onclick="menuChanger(2,this)" class="tab-button-off-link">维度与数据表关系</a></td>
          <td width="5"><img src="<%=request.getContextPath()%>/images/common/taber/tab_right_off.gif" width="5" height="22"></td>
        </tr>
      </table></td>
...
<table width="100%">
	...
	<%
		...
		if ((o_Str != null) && !"".equals(o_Str)) {
			...
			ErrNo = DimDefDaoImpl.setDimTable(request, dim_id, rgChkArr);
...
<TABLE width="100%" height="85%" frameborder="0" marginwidth="0"
	...
	<TR>
<TD><IFRAME name='dimFrame'	src="checkboxDim.jsp?dim_id=<%=dim_id%>"
	...
	%>
<form action="dimTable.jsp" name="hdform" method="post">
<input name="dim_id" type="hidden" value="<%=dim_id%>"/>
<input name="dim_name" type="hidden" value="<%=dim_name%>"/>
