	...
	<xsl:element name="tree">
		<xsl:for-each select="Tree/tree">
			<xsl:element name="tree">				
						...
						<xsl:attribute name="src">createDpregionTreeXML.rptdo?dept_parent=<xsl:value-of select="REGION_ID"/></xsl:attribute>
						<xsl:attribute name="action">javascript:getDept('<xsl:value-of select="REGION_NAME"/>', '<xsl:value-of select="REGION_ID"/>','<xsl:value-of select="REGION_NAME"/>','<xsl:value-of select="REGION_ID"/>');</xsl:attribute>
