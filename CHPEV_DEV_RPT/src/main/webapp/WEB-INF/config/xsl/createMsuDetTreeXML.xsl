			...
			<xsl:element name="tree">
				<xsl:choose>
					<xsl:when test="CATID='1'">
						...
						<xsl:attribute name="src">createMsuDetTreeXML.rptdo?msu_type_id=<xsl:value-of select="MSU_TYPE_ID"/></xsl:attribute>
				...
				<xsl:choose>
					<xsl:when test="CATID='2'">
						...
						<xsl:attribute name="action">msuDet.rptdo?msu_id=<xsl:value-of select="MSU_ID"/></xsl:attribute>
