			...
			<xsl:element name="tree">
				<xsl:choose>
					<xsl:when test="CATID='0'">
						...
						<xsl:attribute name="src">createRolesXML.rptdo?role_type=<xsl:value-of select="CODE_ID"/>&amp;oper_type=1</xsl:attribute>
						<xsl:attribute name="action">javascript:alert('请选择下级角色')</xsl:attribute>
				...
				<xsl:choose>
					<xsl:when test="CATID='1'">
						<xsl:attribute name="text"><xsl:value-of select="ROLE_NAME"/></xsl:attribute>
						<xsl:attribute name="target">work_frame</xsl:attribute>
						<xsl:attribute name="action">javascript:parent.roleDefPage.pageRedirect_id('<xsl:value-of select="ROLE_ID"/>','<xsl:value-of select="ROLE_NAME"/>','<xsl:value-of select="ROLE_TYPE"/>','<xsl:value-of select="LOCAL_NET"/>','<xsl:value-of select="COMMENTS"/>')</xsl:attribute>
