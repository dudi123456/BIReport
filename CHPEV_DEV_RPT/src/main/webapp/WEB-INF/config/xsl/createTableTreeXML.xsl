			...
			<xsl:element name="tree">
				<xsl:choose>
					<xsl:when test="CATID='1'">
						<xsl:attribute name="text"><xsl:value-of select="LAYER_NAME"/></xsl:attribute>
						<xsl:attribute name="target">table_frame</xsl:attribute>
						<xsl:attribute name="src">createTableTreeXML.rptdo?layer_id=<xsl:value-of select="LAYER_ID"/></xsl:attribute>
						<xsl:attribute name="action">javascript:alert('请选择数据表！')</xsl:attribute>
				...
				<xsl:choose>
					<xsl:when test="CATID='2'">
						<xsl:attribute name="text"><xsl:value-of select="DOMAIN_NAME"/></xsl:attribute>
						<xsl:attribute name="target">table_frame</xsl:attribute>
						<xsl:attribute name="src">createTableTreeXML.rptdo?layer_id=<xsl:value-of select="LAYER_ID"/>&amp;domain_id=<xsl:value-of select="DOMAIN_ID"/></xsl:attribute>
						<xsl:attribute name="action">javascript:alert('请选择数据表！')</xsl:attribute>
				...
				<xsl:choose>
					<xsl:when test="CATID='3'">
						<xsl:attribute name="text"><xsl:value-of select="TABLE_NAME"/></xsl:attribute>
						<xsl:attribute name="target">tablelist_frame</xsl:attribute>			
						<xsl:attribute name="action">tableDef.rptdo?table_id=<xsl:value-of select="TABLE_ID"/></xsl:attribute>
