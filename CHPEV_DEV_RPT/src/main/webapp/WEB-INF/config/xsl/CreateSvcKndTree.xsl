	...
	<xsl:element name="tree">
				<xsl:choose>
					<xsl:when test="LVL='1'">
						...
						<xsl:attribute name="src">CreateSvcKndTreeXML.rptdo?svc_parent=<xsl:value-of select="ID"/>&amp;level=<xsl:value-of select="LVL"/></xsl:attribute>
						<xsl:attribute name="icon">../images/common/xmltree/xp/region.gif</xsl:attribute>
        				<xsl:attribute name="action">javascript:parent.getSvcKndData('<xsl:value-of select="NAME"/>', '<xsl:value-of select="ID"/>','<xsl:value-of select="LVL"/>');</xsl:attribute>
