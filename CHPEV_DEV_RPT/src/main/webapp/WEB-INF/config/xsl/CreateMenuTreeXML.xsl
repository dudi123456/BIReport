			...
			<xsl:element name="tree">
				<xsl:choose>
					<xsl:when test="CATID='3'">
						...
						<xsl:attribute name="src">CreateMenuTreeXML.rptdo?menu_code=<xsl:value-of select="MENU_ID"/></xsl:attribute>
						<xsl:attribute name="action">javascript:getMenu('<xsl:value-of select="MENU_NAME"/>', '<xsl:value-of select="MENU_ID"/>')</xsl:attribute>
