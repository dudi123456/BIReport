	...
	<xsl:element name="tree">
		<xsl:for-each select="Tree/tree">
			<xsl:element name="tree">
						...
						<xsl:attribute name="src">createSysDeptTreeXML.rptdo?dept_parent=<xsl:value-of select="DEPT_ID"/></xsl:attribute>
						<xsl:attribute name="action">deptview.jsp?dept_id=<xsl:value-of select="DEPT_ID"/>&amp;region_id=<xsl:value-of select="AREA_ID"/></xsl:attribute>
						<xsl:if test="FLAG='0'">
