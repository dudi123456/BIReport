			...
			<xsl:element name="tree">
			  <xsl:choose>
					<xsl:when test="CATID='3'">
						...
						<xsl:attribute name="src">CreateMenuXML.rptdo?menu_id=<xsl:value-of select="MENU_ID"/></xsl:attribute>
						<xsl:attribute name="action">menuview.jsp?menu_id=<xsl:value-of select="MENU_ID"/></xsl:attribute>
