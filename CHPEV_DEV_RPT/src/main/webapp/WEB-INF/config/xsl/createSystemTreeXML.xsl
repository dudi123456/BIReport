			...
			<xsl:element name="tree">
				<xsl:choose>
					<xsl:when test="CATID='1'">
						<xsl:attribute name="text"><xsl:value-of select="SYSTEM_NAME"/></xsl:attribute>
						<xsl:attribute name="target">systemlist_frame</xsl:attribute>			
						<xsl:attribute name="action">system.rptdo?system_id=<xsl:value-of select="SYSTEM_ID"/></xsl:attribute>
