			...
			<xsl:element name="tree">
			  <xsl:choose>
					<xsl:when test="CATID='1'">
						...
						<xsl:attribute name="src">CreateRegionRoleXML.rptdo?region_id=<xsl:value-of select="REGION_ID"/></xsl:attribute>
						<xsl:attribute name="action">resRoleList.jsp?region_id=<xsl:value-of select="REGION_ID"/></xsl:attribute>
