...
<body>
<form name="form1" action="AdhocInfoExport.rptdo?oper_type=qrySubjectTaskSts" method="post">
<input type="hidden" name="sessionid" value="">
<input type="hidden" name="typeId" value="<%=qryStruct.dim4%>">
<input type="hidden" name="resId" value="<%=qryStruct.dim5%>">

<%=WebPageTool.pageScript("form1","SubjectUserXlsTaskStatus.screen")%>
<% 
...
<TD><%=WebPageTool.pagePolit(pageInfo,rootPath)%></TD>
  ...
  %>  
						...
						<tr class="celdata" onmouseover="this.className='mouseM'" onmouseout="this.className='celdata'">
							...
							<td class="leftdata"><span id="infospan<%=value[0]%>" name="infospan<%=value[0]%>"><%if (flag==2){%><a href="javascript:;" onclick="doDownLoadFile('<%=value[5]%>','<%=rootPath%>','<%=value[9]%>')">下载</a><%}else if(flag==3){%><%=value[7]%><%}%>&nbsp;&nbsp;<%if (flag==2){%><a href="javascript:;" onclick="deleteJob('<%=value[0]%>')">删除</a>&nbsp;&nbsp;<%}if (flag==0){%><a href="javascript:;" onclick="qryTaskQueue('<%=value[0]%>','<%=value[9]%>');">查看任务队列</a><%}%></span></td>
