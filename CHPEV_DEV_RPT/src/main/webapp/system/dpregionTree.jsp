...
<TD>
<script>
var tree = new WebFXLoadTree("<%=tree_name%>","createDpregionTreeXML.rptdo?user_id=<%=user_id%>");
document.write(tree);
</script>
</TD>
</TR>
...
function linkUrl(){
   //parent.getDept('<%=tree_name%>', '<%=dept_parent%>','<%=info_dept.area_name%>','<%=info_dept.area_id%>');
}
</script>
