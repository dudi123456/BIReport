...
<script language="javascript">
	...
	<%
	        ...
	        List<PassInfo> passlist = ( List<PassInfo>)	session.getAttribute("tacticPassList");
			...
			TacticInfo tacInfo = (TacticInfo)session.getAttribute("TacticInfo");

	%>
</head>
<body onload="myLoad()">
<form action="" name="form2" id="form2" method="post">
<input type="hidden" value="<%=tacInfo.getTacticId() %>" id="object_id" name="object_id">
<input type="hidden" value="tacticPassList" id="optype" name="optype">
<input type="hidden" value="tactic" id="doType" name="doType">
<input type="hidden" value="tactic" id="state" name="state">
<input type="hidden" value="tactic" id="advice" name="advice">

<%
	if(null!=tacInfo){
		if(0==tacInfo.getState()){
%>
 		&nbsp;此方案还没有提交  &nbsp  请选择审批人： <BIBM:TagSelectList 	listName="warn_name" listID="0"  selfSQL="<%=personName%>" />
        &nbsp;&nbsp; <button class="btn4" type="button" onClick="mySubmit()"> 提交 </button>
<%
		}else {
	%>
				<%
				if(1==tacInfo.getState()&&loginUser.user_id.equals(tacInfo.getDecider()) ){
						%>
