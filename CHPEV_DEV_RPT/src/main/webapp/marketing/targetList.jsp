...
<script language="javascript">
	...
	function myAdd(op) {
		form1.action="targetAction.rptdo?optype=targetAdd&doType="+op;
		form1.submit();
	}
	function myModify(op) {
		...
		if(checlCount==0){
			...
			form1.action="targetAction.rptdo?optype=targetAdd&doType="+op+"&targetId="+id;
			form1.submit();
		}
	...
	<%
		...
		//获得审批意见类型
		String targetStateSql = "select t.code_id,t.code_name from ui_code_list t  where t.type_code='MK_PL_APPROVAL_TYPE'";
...
<form name="form1" method="post" action="">
  <div id="maincontent">
     <div class="toptag" > 您所在位置：营销策划 >> 营销策略 >> <em class="red">营销指标管理</em> <span class="hbt"><a href="javascript:;"
    ...
    <div class="topsearch">
      <table width="100%">
        ...
        <tr>
          <td align="right" width="15%">指标名称： </td>
          <td><input id=qry__target_name name=qry__target_name  value="<%=qryStruct.target_name %>" class="txtinput"  style="width:150px"/></td>
          <td align="right" width="15%">指标类型： </td>
          <td>
          ...
          <td>
         	 <BIBM:TagSelectList focusID="<%=qryStruct.target_state%>" script="class='easyui-combobox'"	listName="qry__target_state" listID="0" allFlag="" selfSQL="<%=targetStateSql %>" />
          </td>
        ...
        <% if(null!=list)
        	{
        		for(int i = 0 ;i<list.size();i++){
        			...
        			%>
        			 <tr><%} %>
          <td align="center"><label> <input type="checkbox" id="checkbox_<%=list.get(i).getTargetId() %>" name="checkbox" value="<%=list.get(i).getTargetId() %>"></label></td>
