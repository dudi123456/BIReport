...
function doSearch()
{
	var qryPackageName=document.getElementById("qry_package_name").value;
 	var qryPackageType=document.getElementById("qry_package_type").value;
 	if (true) {
 	var url = "packageManaAction.rptdo?doType=search&optype=searchPackagedialog&qry_package_name="+qryPackageName+"&qry_package_type="+qryPackageType;
 	form1.action=url;
 	form1.submit();}
}
<%
List<PackageInfo> list = (List<PackageInfo>) request.getAttribute("packageList");
out.println(list == null ? 0:list.size());
...
<!-- 弹出模式窗口 -->
<input type="hidden" id="qry__package_state" name="qry__package_state" value="1">
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="validateBottom">
        <tr>
          <td width="15%" class="validatebox-tabTD-left">套餐名称：</td>
          <td width="30%" class="validatebox-tabTD-right"><input value="<%=qryStruct.package_name%>" id="qry_package_name" name="qry_package_name"></td>
          <td width="15%" class="validatebox-tabTD-left">套餐类型：</td>
          <td width="30%" class="validatebox-tabTD-right">
          <BIBM:TagSelectList  focusID="<%=qryStruct.package_type%>" allFlag=""	listName="qry_package_type" listID="0" selfSQL="<%=packagetypeSql%>" />
          </td>
 ...
 <div class="list_content">
      <table >
        <tr width="100%">
          ...
          <th width="12%" align="center"> 套餐名称 </th>
         ...
         <% if(null!=list)
          ...
          <td align="center"><label>
            <input type="checkbox" id="checkbox_<%=list.get(i).getPackageId()%>" name="checkbox" value="<%=list.get(i).getPackageId() %>@@@<%=list.get(i).getPackageName() %>">
          </label></td>
          <td align="center"><%=list.get(i).getPackageName()%></td>
          <td align="center"><%=CodeParamUtil.codeListParamFetcher(request,"MK_PL_PACKAGE_TYPE",String.valueOf(list.get(i).getPackageType()))  %></td>
