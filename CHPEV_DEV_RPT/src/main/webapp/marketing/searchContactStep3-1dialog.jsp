...
function mychange(){
	form1.action="surveyAction.rptdo?optype=searchContactStep3-2dialog&doType=search&qry__survey_state=2";
	form1.submit();
}
<%
ReportQryStruct qryStruct = (ReportQryStruct) session.getAttribute(WebKeys.ATTR_SUBJECT_QUERY_STRUCT);
String typeSql ="select t.code_id,t.code_name from ui_code_list t  where t.type_code='MK_PL_MSG_TYPE'";
String opModeSql = "select t.code_id||'@@@'||t.code_name,t.code_name from ui_code_list t  where t.type_code='MK_PL_CONTAC_MODE' order by  t.code_id desc";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="validateBottom">
        <tr>
         <td width="15%" class="validatebox-tabTD-left">接触方式：</td>
          ...
          <td width="25%" class="validatebox-tabTD-right">
		   <input type="hidden" value="2" id="qry__msg_state" name="qry__msg_state">
...
<%if(null!=list)
        	{
        	  for(int i=0;i<list.size();i++){
        			...
        			%>
        			 <tr><%} %>
          <td align="center"><label> <input type="checkbox" id="checkbox_<%=list.get(i).getMsgId() %>" name="checkbox" value="<%=list.get(i).getMsgId() %>@@@<%=list.get(i).getContent() %>"></label></td>
          <td align="left"><%=list.get(i).getContent()  %></td>
           <td align="center"><%=list.get(i).getCreator() %></td>
          <td align="center"><%=CodeParamUtil.codeListParamFetcher(request,"MK_PL_MSG_TYPE",String.valueOf(list.get(i).getMsgType()))  %></td>
          <td align="center"><%=CodeParamUtil.codeListParamFetcher(request,"MK_PL_APPROVAL_TYPE",String.valueOf(list.get(i).getMsgState()))  %></td>
 		  <td align="center" class="last"><%=CommonFormate.dateFormate(list.get(i).getCreateTime()) %></td>
        </tr>
        			<%
        		}
        	}%>
