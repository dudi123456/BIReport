...
function doSearch()
{
 	form1.action="projectAction.rptdo?optype=searchProjectdialog&doType=search";
 	form1.submit();
}
<%
List<ProjectInfo> list = (List<ProjectInfo>) session.getAttribute("ProjectInfoList");
...
<!-- 弹出模式窗口 -->
<input type="hidden" id="qry__project_state" name="qry__project_state" value="2">
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="validateBottom">
        <tr>
          <td width="15%" class="validatebox-tabTD-left">方案名称：</td>
          <td width="30%" class="validatebox-tabTD-right"><input value="<%=qryStruct.project_name%>" id="qry__project_name" name="qry__project_name"></td>
          <td width="15%" class="validatebox-tabTD-left">方案类型：</td>
          <td width="30%" class="validatebox-tabTD-right">
          <BIBM:TagSelectList  focusID="<%=qryStruct.project_type%>" allFlag=""	listName="qry__project_type" listID="0" selfSQL="<%=tactictypeSql%>" />
          </td>
 ...
 <div class="list_content">
      <table >
        <tr width="100%">
          ...
          <th width="12%" align="center"> 方案名称 </th>
         ...
         <% if(null!=list)
          ...
          <td align="center"><label>
            <input type="checkbox" id="checkbox_<%=list.get(i).getProjectId() %>" name="checkbox" value="<%=list.get(i).getProjectId() %>@@@<%=list.get(i).getProjectName() %>">
          </label></td>
          <td align="center"><%=list.get(i).getProjectName() %></td>
          <td align="center"><%=CodeParamUtil.codeListParamFetcher(request,"MK_PL_TACTIC_TYPE",String.valueOf(list.get(i).getProjectType()))  %></td>
