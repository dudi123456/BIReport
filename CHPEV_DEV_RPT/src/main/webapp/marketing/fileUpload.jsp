...
<body style="font-size: 12px" >
 <form action="fileUploadAction.rptdo?optype=fileUpload&doType=add" method="post" enctype="multipart/form-data" name="form1">
...
<%
 	if(null!=list){
          ...
          <%=list.get(i).getFileCode()%>
          </a>
          </td>
      	  <td align="center"><%=list.get(i).getFileName()%></td>
          <td align="center"><%=list.get(i).getFileSize()%></td>
          <td align="center"><a style= "cursor:hand" href="fileUploadAction.rptdo?optype=fileUpload&doType=delete&fileCode=<%=list.get(i).getFileCode()%>">删除</a></td>
       </tr>	<%
       		}
       	}%>
     </table>
