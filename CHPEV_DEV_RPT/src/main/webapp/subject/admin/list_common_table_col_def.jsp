...
function _editRptTableDefDrill(rpt_id,rowid){
  var optstr = "height=680,width=800,left=0,top=0,status=no,toolbar=no,menubar=no,location=no,scrollbars=yes,resizable=no";
  var strUrl = "SubjectCommonTblDef.rptdo?opt_type=edtTblColDef&table_id=" + rpt_id + "&rowid=" + rowid;
  newsWin = window.open(strUrl,"editRptHead",optstr);
  if(newsWin!=null){
    newsWin.focus();
  }
...
function addNewInfo(rpt_id){
  var optstr = "height=680,width=800,left=0,top=0,status=no,toolbar=no,menubar=no,location=no,scrollbars=yes,resizable=no";
  var strUrl = "SubjectCommonTblDef.rptdo?opt_type=addTblColDef&table_id=" + rpt_id;
  newsWin = window.open(strUrl,"editRptHead",optstr);
  if(newsWin!=null){
    newsWin.focus();
  }
...
function _delete(rpt_id,rowid){
 if(confirm("您确定要删除吗？此操作不可恢复!")){
 	...
 	var strUrl = "SubjectCommonTblDef.rptdo?opt_type=doDelCommTblColDef&table_id=" + rpt_id + "&rowid=" + rowid;
...
<TABLE width="100%" border="0" cellpadding="0" cellspacing="1" class="table-bg">
  <tr align="center">
    <td class="tab-title" width="10%">列名称</td>
    <td class="tab-title"  width="10%">列描述</td>
	<td class="tab-title" width="10%">列ID</td>
    <td class="tab-title"  width="10%">显示顺序</td>
    <td class="tab-title" width="10%">源字段ID</td>
    <td class="tab-title" width="10%">源字段描述</td>    
  ...
  UiSubjectCommonColDef obj = (UiSubjectCommonColDef)listInfo.get(i);
  %>
  <tr class="table-white-bg">
  <td nowrap><%=obj.getColName()%></td>
	<td nowrap><%=obj.getColDesc()%></td>
  <td nowrap><%=obj.getColId()%></td>
	<td nowrap><%=obj.getColSequence()%></td>

	<td nowrap><%=obj.getCodeField()%></td>
  <td nowrap><%=obj.getDescField()%></td>
