...
public class CreateFavorTreeXmlHTMLAction extends HTMLActionSupport {
	/**
	 * 
	 */
	...
	public void doTrans(HttpServletRequest request, HttpServletResponse response)
		...
		if (favor_id != null && !"".equals(favor_id)) {
			try {
				sql = SQLGenator.genSQL("fav002", loginUser.user_id, favor_id,
						loginUser.user_id, favor_id);
			} catch (AppException ex2) {
				ex2.printStackTrace();
			}
		} else {
			try {
				sql = SQLGenator.genSQL("fav001", loginUser.user_id, favor_id);
			} catch (AppException ex2) {
				ex2.printStackTrace();
			}
		}
		...
		try {
			...
			if (result == null) {
				throw new HTMLActionException(session,
						HTMLActionException.WARN_PAGE, "收藏夹信息失败！");
