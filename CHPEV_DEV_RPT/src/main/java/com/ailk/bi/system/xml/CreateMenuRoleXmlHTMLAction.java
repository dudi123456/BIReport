...
public class CreateMenuRoleXmlHTMLAction extends HTMLActionSupport {
	...
	public void doTrans(HttpServletRequest request, HttpServletResponse response)
		...
		if (menu_id != null && !"".equals(menu_id)) {
			try {
				sql = SQLGenator.genSQL("Q2935", loginUser.system_id,
						loginUser.user_id, loginUser.user_id, menu_id);
		...
		/*
		 * if(menu_id != null && !"".equals(menu_id)){ try { sql =
		 * SQLGenator.genSQL("Q2937", menu_id); } catch (AppException ex2) {
		 * ex2.printStackTrace(); } }else{ try { sql =
		 * SQLGenator.genSQL("Q2938"); } catch (AppException ex2) {
		 * ex2.printStackTrace(); } }
		 */
		try {
			// 取sql语句
