...
public class CreateDpGroupTreeXmlHTMLAction extends HTMLActionSupport {
	/**
	 *
	 */
			...
			if (parent != null && !"".equals(parent)) {
				...
				try {
					if ("1".equals(user.group_id) || "2".equals(user.group_id)) {
						sql = SQLGenator.genSQL("A00041", user.system_id);
					} else {
						sql = SQLGenator.genSQL("A00043", user.user_id);
					}
				} catch (AppException ex2) {
					ex2.printStackTrace();
				}
