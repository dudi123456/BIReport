...
import com.ailk.client.filter.interf.SessionLoadHandler;

public class ChpevDevSessionLoadHandler implements SessionLoadHandler {
	...
	public void loadSession(HttpServletRequest req, HttpServletResponse res) throws Exception {
		...
		Assertion assertion = (Assertion) (session == null ? req
		...
		if (assertion != null) {
			...
			Assertion casLoginUser = (Assertion) (session == null ? req
					.getAttribute(AbstractCasFilter.CONST_CAS_ASSERTION)
					: session
							.getAttribute(AbstractCasFilter.CONST_CAS_ASSERTION));
	...
	private Map<String, Object> getUserInfoDetail(String userCode) throws AppException {
        ...
        List<Map<String, Object>> userInfoDetail = WebDBUtil.execQryArrayMapSys(sql.toString(),params);
