...
public class AjaxHttpRequest {
	...
	public void open(final String method, final URL url, boolean async,
		...
		this.abort();
		synchronized (this) {
			this.connection = this.connection == null ? url.openConnection()
					: this.connection;
			;
			this.async = async;
	...
	/**
	 * 类似 XMLHttpRequest 中的 responseXML 属性. 而且需要两个额外的包,两个包不是所有的java环境都有的.
	 * 所以我(fins)把此方法注释条了. 如果确实需要此方法 请自行取消该方法的注释,并引入下面列出的类/接口.
	...
	protected void changeState(int readyState, int status,
		...
		int oReadyState = this.readyState;
		synchronized (this) {
		...
		if (this.readyStateChangeListener != null && oReadyState != readyState) {
